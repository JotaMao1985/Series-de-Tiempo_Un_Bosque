...
rejilla <- lapply(rejilla_def, function(pq) {
  ...
  list(
    ...
    aic    = round(as.numeric(aj$aic), 3),
    aicc   = round(aicc_de(aj, n_m), 3),
    bic    = round(as.numeric(BIC(aj)), 3),
...
transformacion <- list(
  ...
  sqrt = list(
    ...
    aicc = round(aicc_de(ml_sqrt, n_m), 3),
    bic  = round(as.numeric(BIC(ml_sqrt)), 3),
